...
import org.apache.spark.deploy.types.SesCMPaperTypes.W.PDriver_w_W.RcvLaunchDriver
import org.apache.spark.deploy.types.SesCMPaperTypes.W.PExSchedule_w_W.RcvStartExCase
...
object PiFailureDriverInExSchedule extends TestAppUtil {
  val outDir = "output/test/failureDriverExSchedule"
  ...
  def main(args: Array[String]): Unit = {
   ...
   val wcustomCode: (__EPType_W, StateW, AbstractChannelImp) => Unit = {
     case (RcvStartExCase,_,_) if !failed.get() =>
       failed.set(true)
       assert(false)
     case _ =>
   }
