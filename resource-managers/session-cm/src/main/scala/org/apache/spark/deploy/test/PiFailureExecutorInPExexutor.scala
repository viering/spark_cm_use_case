...
import org.apache.spark.deploy.types.SesCMPaperTypes.W.PExecutor_wEx_W.RcvStartEx
...
object PiFailureExecutorInPExexutor extends TestAppUtil {
  val outDir = "output/test/failureExecutorinPexecutor"
  ...
  def main(args: Array[String]): Unit = {
   ...
   val wcustomCode: (__EPType_W, StateW, AbstractChannelImp) => Unit = {
     case (RcvStartEx,_,_) if !failed.get() =>
       failed.set(true)
       assert(false)
     case _ =>
   }
