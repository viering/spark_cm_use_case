...
object TpchQuery {
  ...
  def outputDF(df: DataFrame, outputDir: String, className: String): Unit = {
    println(s"output path: ${outputDir + "/" + className}")
    val res = df.collect()
    res.foreach(println)

    var bw: BufferedWriter = null
    try {
      val file = new File(new File(".").getAbsolutePath(),className + ".dat")
      bw = new BufferedWriter(new FileWriter(file))
      res.foreach(l => bw.write(l.mkString(",") + "\n"))
    } catch {
